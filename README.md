...
сохранении. **Булева анимация состояния**: тумблер «Булево состояние (вкл/выкл)» + на выделенной фигуре
видимость `Всегда / При вкл / При выкл` (суффиксы `.true` / `.false`, эффект
`animation-hidden`), превью `Все / Вкл / Выкл` на холсте; на выходе — булев `onoff`-слот

- `animationTemplate`. Серость (де-энергизация) в стенсиле не задаётся — это холст
  (`switchSources`). Стенсил регистрируется в реестре сразу (появляется в палитре) и
