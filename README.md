...
В multi-select — массовое применение «Диапазонов значений» / «Булева источника»
к выделению; стенсилы с `layoutOnly: true` (cell_text / cell_value)
...
наследует voltage/switch провода. Во время drag над проводом превью липнет к
линии и поворачивается под угол врезки.
...
контекстное меню, оверлей-кнопка, инспектор). В multi-select срастание НЕ
делаем — туда авто-попадают мостовые провода между ячейками, сохранять нечего.
