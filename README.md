...
Правка — кнопка-карандаш в строке стенсила (по ховеру, **только у `userCreated`**;
родные read-only, их SVG не в нашем формате). Открывает редактор с загруженным
стенсилом (`parseStencilSvg` разбирает `shape.svg` обратно в примитивы, порты — из
`stencil.json`); id заблокирован (= имя папки). На сохранении переинжектит активную
форму — расставленные экземпляры подхватывают новый рисунок; при смене размера/портов
предупреждает (их геометрия у экземпляров не трогается).

Удаление — кнопка-корзина в строке стенсила (по ховеру, **только у `userCreated`**;
родные удалить нельзя) с ConfirmPopup: снимает из реестра + сносит папку. Если стенсил
расставлен в формах, удаление отклоняется (не осиротить ячейки), с указанием где.
