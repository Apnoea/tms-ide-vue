...
- `stencil.json` — id, label, category, size, ports + опционально slots,
  animationTemplate, `states` (анимация «по значению»), minWidth (только cell_bus)
...
Превью состояния — плавающий Select слева-сверху на холсте (эмуляция видимости, на
экспорт не влияет). Чекбокс `quality` (серость + «показать все положения» при
bad-качестве) — в том же блоке анимации, только при включённой анимации.
Цвет/серость (де-энергизация) в стенсиле не задаётся — это холст (`switchSources`/
`voltageSource`). Стенсил регистрируется в реестре сразу (появляется в палитре) и
