...
(кратно 10) и сдвигается в (0,0) — «воздух» холста отбрасывается. Метаданные
(id `[a-z0-9_]`, label, категория, размер), флаги поведения (`noRotate` /
`layoutOnly` / `quality` — чекбоксы) и цвет линии / заливка выделенной фигуры правятся
в правой панели (`StencilInspector`, вместо инспектора ячейки) и валидируются при
сохранении. **Анимация состояния** (единый свитч «Выкл / Булево / По значению», один режим за раз):
...
Превью состояния — Select в тулбаре редактора (эмуляция видимости, на экспорт не влияет).
