...
отбрасывается. Метаданные (id `[a-z0-9_]`, label, категория, размер) и флаги
поведения (`noRotate` / `layoutOnly` / `quality` — чекбоксы) правятся в правой
панели (`StencilInspector`, вместо инспектора ячейки) и валидируются при
сохранении; стенсил регистрируется в реестре сразу (появляется в палитре) и
пишется в `definitions/<id>/` dev-плагином.

Правка — кнопка-карандаш в строке стенсила (по ховеру, у всех **кроме залоченных**
`locked: true`; ими помечены программные / анимированные / с текстом — их SVG в наш
формат не разбирается). Открывает редактор с загруженным стенсилом (`parseStencilSvg`
