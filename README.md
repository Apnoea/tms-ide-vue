...
в этом состоянии перекрашиваются **контуры символа** (только `stroke` — заливку не
трогаем, чтобы не превращать символ в сплошной «блоб»); пусто → состояние меняет
только видимость фигур. На выходе — `stateColors` в
...
(горизонт./вертик.) относительно рамки выделения, плюс 2 кнопки **распределения**
(равные интервалы по горизонтали/вертикали, при ≥3 ячейках). Наложений не создаёт:
ячейки, пересекающиеся по перпендикуляру, пакуются вплотную (сводятся до касания);
