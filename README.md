...
│   └── useWorkspaceStore.js   # формы проекта + активная + дерево форм (formTree)
├── stencils/
...
по узлу → форма на холсте). Иерархия узлов хранится в `hierarchy.json` проекта
(v1 — только чтение, без drag-перестройки). Состояние проекта (формы пер-id,
активная, теги, дерево) хранится в IndexedDB и переживает перезагрузку.
...
- **Формы** — дерево в левой панели: «+» в шапке создаёт пустую (падает
  ...
  (единственную удалить нельзя). Формы вне дерева показываются группой «Без
...
- **Экспортировать проект** (`Ctrl+S`) — скачивание одного **.zip** (той же
  раскладки): по форме `view.svg`+`animations.json`, используемые стенсилы в
