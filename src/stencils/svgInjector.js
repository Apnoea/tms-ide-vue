 ...
 * ширине (computeBusPorts), остальные берут из stencil.ports. `magnet: 'passive'` у
 * cell_node: подключаться к узлу можно, тащить от него нельзя.
 */
 ...
 * определения, поэтому увеличенный экземпляр растягивается трансформом, а `shape.svg`
 * не перерисовывается. Отражение считается в ИТОГОВЫХ размерах и идёт первым, масштаб
 * вторым. Одна функция на холст и экспорт.
 *
 * Обводка масштабируется вместе с рисунком (`vector-effect="non-scaling-stroke"` не
 * годится: он привязывает толщину к экрану, и зум холста перестал бы её менять).
 *
...
export function contentScales(stencil) {
  ...
  // Шина (слоты по ширине), подпись (габарит по тексту) и точка соединения (диаметр
  // полем) рисуются от фактического размера. Карточки значения здесь нет: она рисуется
  // в координатах определения и масштабируется как обычный символ.
 ...
 * такой конец ОТЦЕПЛЯЕТСЯ в точку, где порт был. Перевешивать на ближайший нельзя:
 * неверное соединение на мнемосхеме выглядит рабочим.
 *
 * Габарит задаёт определение символа (× `tms.scale` экземпляра) — кроме тех, у кого он
 * свой: шина (ресайз, `minWidth`) и подписи/значения (`static`). Если известна прежняя
 * версия символа, критерий точнее: размер, совпадавший с прежним, считается дефолтным,
 * отличавшийся — выставленным пользователем. Масштаб сохраняется: ×2 остаётся ×2.
 *
 ...
 *
 * Провода приводятся к своей полосе z (`fromJSON` шлёт `reset`, add-хендлер молчит, а
 * у импортированных проектов z нет). Именно нормализация, а не константа: порядок
 * внутри полосы задаёт пользователь. Запись только при расхождении — иначе z дрейфит
 * и ломает undo/redo.
 *
 * `sync: true` — ЗАГРУЗКА формы (restore, смена формы, импорт, прогон при экспорте):
 * заодно сверяются порты и габарит с реестром, ведь символ могли править, пока форма
 * была закрыта. Для undo/redo флаг НЕ ставится: там граф обязан стать ровно снимком.
 *
