...
// строит busCell — здесь только выбор нужного билдера.
 ...
 * (шина: слот каждые BUS_PORT_SPACING). Сверка с реестром такие не трогает — их набор
 * держит `useBusResize.syncBusPorts`.
 */
...
/**
 * Размер экземпляра при масштабе. Обе стороны снапятся к сетке холста: на клетках
 * обязаны стоять и габарит, и крайние порты, иначе концы проводов слезают с сетки.
 * Цена — коэффициенты по осям расходятся до половины клетки.
 */
 ...
 * размер) — округление к ближайшей клетке увело бы вывод внутрь тела. Внутренние
 * снапятся к сетке.
 */
 ...
 * ширине (computeBusPorts), остальные берут из stencil.ports. `magnet` из определения
 * переносим как есть ('passive' — подключаться можно, тащить от порта нельзя).
 */
export function buildPortItems(stencil, width, height, flip = {}) {
  ...
  return (stencil.ports || []).map((p) => {
    // Координаты в определении абсолютные — у масштабированного экземпляра их надо
    // пересчитать.
...
/**
 * Привести порты ячейки к `items`. Только через port-manager: `set('ports', {items})`
 * заменяет объект целиком и сносит `groups` из defaults `TMSStencil`, после чего
 * JointJS падает на расчёте позиций.
 *
...
/**
 * Трансформация КОНТЕНТА символа: отражение (в ИТОГОВЫХ размерах, первым) плюс
 * масштаб. Рисунок лежит в координатах определения и растягивается трансформом, а
 * `shape.svg` не перерисовывается. Одна функция на холст и экспорт.
 *
 * Обводка масштабируется вместе с рисунком: `vector-effect="non-scaling-stroke"`
 * привязал бы толщину к экрану, и зум холста перестал бы её менять.
 *
...
// Коэффициент — результат деления: режем хвост float'а, он уезжает в атрибут SVG.
function round3(v) {
...
export function injectStencilSvg(cellView, stencil) {
  ...
  // Шина программная: размер и содержимое задаёт tms, а не shape.svg.
...
/**
 * Ячейка-символ в графе: порты с учётом flip, модель, инъекция SVG — единая точка для
 * drop'а из палитры и для paste. Позицию, размер, tms и angle готовит вызывающий.
 */
...
/**
 * Нарисован ли контент символа в координатах ОПРЕДЕЛЕНИЯ. У программных билдеры рисуют
 * по ФАКТИЧЕСКОМУ размеру экземпляра, поэтому трансформ растянул бы тело второй раз.
 */
export function contentScales(stencil) {
  if (!stencil?.width || !stencil?.height) return false
  // Шина (слоты по ширине) и подпись (габарит по тексту) рисуются от фактического
  // размера. Карточки значения здесь нет: она рисуется в координатах определения и
  // масштабируется как обычный символ.
  return stencil.id !== 'cell_bus' && !stencil.minWidth
}

/**
 * Масштабируется ли символ ручками. Исключение — шина: габарит у неё свой (ресайз).
 * Замок гейтит вызывающий (useCanvasResize).
 *
 ...
 * контента. Позицию считает вызывающий (у каждой ручки свой фиксированный угол).
 *
...
export function applyStencilScale(cell, paper, scale, { position } = {}) {
  ...
  // ×1 — дефолт: поле не пишем, иначе оно уедет в meta пустым фактом.
...
/**
 * Правка символа в редакторе → расставленные экземпляры на холсте: набор и позиции
 * портов, габарит, рисунок. Провод держится за порт по имени и едет за ним сам.
 *
 * Конец на порту, которого в новой версии нет, ОТЦЕПЛЯЕТСЯ в точку, где порт был.
 * Перевешивать на ближайший нельзя: неверное соединение выглядит рабочим.
 *
 * Габарит задаёт определение (× `tms.scale`), кроме символов со своим размером — шина
 * (`minWidth`) и подписи/значения (`static`). С `prev` критерий точнее: размер,
 * совпадавший с прежним, считается дефолтным, отличавшийся — заданным пользователем.
 *
...
export function syncStencilInstances(graph, paper, stencil, prev = null) {
  ...
  // План по экземплярам считается ДО правок: целевой набор портов зависит от размера
  // ЭКЗЕМПЛЯРА, а не только от определения.
  ...
  // Отцепление идёт ДО пересборки портов: позиция удалённого порта нужна.
  if (!computedPorts) {
 ...
 * знает наших символов, поэтому SVG инъектится заново. `angle` восстанавливать не
 * нужно, его держит outer-`<g>`.
 *
 * Провода приводятся к своей полосе z — нормализацией, а не константой: порядок внутри
 * полосы задаёт пользователь. Пишем только при расхождении, иначе z дрейфит и ломает
 * undo/redo.
 *
 * `sync: true` — ЗАГРУЗКА формы (restore, смена формы, импорт, прогон при экспорте):
 * заодно сверяет порты и габарит с реестром. Для undo/redo флаг НЕ ставится: граф
 * обязан стать ровно снимком.
 *
...
export function reinjectAllStencils(graph, paper, { sync = false } = {}) {
  ...
  // Фигуры-разметка (tms.Shape) после fromJSON тоже пустые: это единая точка
  // «восстановить вид формы».
