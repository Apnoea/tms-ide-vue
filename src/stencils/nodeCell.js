// Точка соединения (cell_node) — программный символ прошлого формата: точка рисуется
// по `tms` (цвет и размер), а не из shape.svg. Геометрия и цвета одни для экспорта
// (buildNodeExportSvg) и холста (buildNodeContent).
//
// Габарит ячейки (20×20) не меняется: он держит hit-area и порт `center`, а
// `defaultAnchor`/`defaultConnectionPoint` холста считают центр bbox.
import { RANGE_FILL_CLASS, cssColor } from '../constants/animation'
import { SVG_NS, svgEl } from '../utils/xml'

/**
 * Цвет и диаметр точки по умолчанию (тот же вид, что в shape.svg — он остался для
 * превью). Минимум диаметра = дефолт: тоньше точка неотличима от пересечения проводов.
 */
...
/**
 * Цвет к отрисовке — чистится здесь, а не у вызывающих: значение уходит в атрибут
 * `fill` экспортного SVG, а в модель могло попасть `url(#…)` из чужого архива.
 */
