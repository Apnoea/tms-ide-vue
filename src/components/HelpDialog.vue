...
// Группированный список клавиш И жестов. Каждая строка: { keys: [...], desc }.
// keys — основной аккорд (клавиши через «+»). keysAlt — альтернативный
// аккорд для того же действия, рисуется через «/» (напр. Del / Backspace).
//
// Справка — ЕДИНСТВЕННАЯ точка, где узнаются возможности без хоткея (врезка в
// провод, сжатие шины, дублирование формы, перенос настроек анимации): в UI они
// обнаруживаются только случайно. Поэтому жесты держим здесь наравне с клавишами,
// а `keys` для них — короткий ярлык жеста, чтобы правая колонка не разъезжалась.
const sections = [
  {
    title: 'Навигация',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      { keys: ['ПКМ', '→ Порядок'], desc: 'Слой символа: вперёд / назад (провода всегда позади)' },
      {
        keys: ['Кнопка', 'замок'],
  ...
  },
  {
    ...
    items: [
      ...
      {
        keys: ['Drag', 'края шины'],
        desc: 'Длина шины; сжатие уберёт провода на исчезнувших слотах',
      },
    ],
  },
  {
...
<template>
  <Dialog
    ...
    :draggable="false"
  >
    <!-- Секций стало много (клавиши + жесты) — ограничиваем высоту и скроллим
         внутри, иначе на 768px-экране диалог вылезал бы за пределы окна. -->
