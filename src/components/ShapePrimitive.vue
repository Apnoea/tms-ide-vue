 ...
 *
 * Halo линии и ломаной УДЛИНЯЕТСЯ на запас: обводка расширяется только перпендикулярно,
 * и рамка равной толщины со всех сторон получается только так.
 *
 * Подпись — отдельная ветка: обводки нет, цвет в `fill`, halo рисуется по замеренному
 * bbox. У ПУСТОЙ подписи вместо глифов иконка и прозрачная область попадания — своей у
 * пустого `<text>` нет, а такая подпись штатна (текст приходит с холста параметром).
 * Роль подписи (значение тега / правится на холсте) помечается у самой фигуры; метки
 * живут только в редакторе и в `shape.svg` не уезжают.
 *
