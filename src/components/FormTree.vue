 ...
 * Клик по узлу — открыть форму, карандаш — inline-rename, × — удаление, «+» в
 * шапке — создать (падает последним узлом в корень), drag-and-drop — перестройка
 * (перенос/вложение узлов). Формы вне дерева → группа «Без иерархии»; узлы на
  ...
  <div class="flex shrink-0 flex-col border-b border-surface-200 bg-surface-50">
    ...
    >
      ...
      <!-- Кнопка «создать форму» — тот же PrimeVue Button, что в тулбаре холста
           (secondary/text/small): единый размер, hover и поведение. -->
      <Button
