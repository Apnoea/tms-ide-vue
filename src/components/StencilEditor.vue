 ...
 * и анимация состояния со снапом к сетке (вершины 1px, порты и размер символа —
 * PORT_GRID). Рисовать можно только в области символа.
 *
...
/**
 * Жест рисующего инструмента. Слушается со STAGE, а не с SVG символа: начинать штрих
 * и ставить порт можно за пределами холста, как и рамку выделения. Координаты считает
 * `unitsFromEvent`, а к области символа их прижимает снап (`snapShapeX/Y` и
 * `portOnEdge` клампят в 0..width/height).
 */
...
/**
 * Старт рамки слушается на всей области просмотра (stage), а не на SVG: когда фигура
 * прижата к краю холста, пустого места внутри нет. Координаты считает unitsFromEvent,
 * за границами viewBox значения выходят за 0..W/H — для рамки это нормально, в модель
 * они не пишутся.
 */
