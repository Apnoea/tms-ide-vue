<script setup>
/**
 * Поле цвета: свотч, палитра в поповере, поле кода и недавние цвета (`useRecentColors`,
 * общие на всё приложение). Нативный `<input type="color">` не годится — его диалог
 * прячет hex и не управляется со страницы.
 *
 * Палитра правит цвет ЖИВЬЁМ (результат виден на схеме сразу), код — по КОММИТУ
 * (Enter / потеря фокуса): промежуточные `#f`/`#f5` цветом не являются. Невалидный
 * ввод откатывается к текущему значению, пустого цвета у поля нет.
 *
 * `change` = жест закончен: по нему пишется шаг истории и запоминается недавний цвет.
 */
