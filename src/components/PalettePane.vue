...
/* `.p-accordioncontent` — GRID (`grid-template-rows: 1fr`), а трек грида по умолчанию
   не уже своего содержимого: длинное название символа распирало список, и вся колонка
   уезжала в горизонтальный скролл, хотя у названия стоит `truncate`. `minmax(0, 1fr)`
   разрешает треку сжиматься — усечение начинает работать. */
.tms-palette-accordion .p-accordioncontent {
