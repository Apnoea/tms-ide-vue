...
/**
 * Ресайз за ручки на холсте: фигуры-разметки и символы. Ручки — HTML-overlay, а не
 * хэндлы внутри ячейки: они нужны только выделенному и не должны попадать в экспорт.
 *
 * Прямоугольник и эллипс тянут за ГАБАРИТ (`resizeShapeCell` масштабирует геометрию
 * под него), противоположный край стоит на месте, минимум — шаг сетки. Линия и ломаная
 * правятся ПО ТОЧКАМ (`moveShapePoint`): ручка на каждой вершине.
 *
 * Символы масштабируются ПРОПОРЦИОНАЛЬНО за угловые ручки: рисунок растягивается
 * трансформом, порты пересчитываются от размера (`applyStencilScale`). Боковых ручек у
 * них нет — растяжение по одной оси искажает SCADA-символ; ниже родного размера не
 * уменьшаем, порты сошлись бы в одну клетку.
 *
 * Повёрнутый СИМВОЛ масштабируется наравне с прямым: масштаб один на обе оси, поэтому
 * вектор жеста поворачивается на −angle, ручки ставятся на визуальные углы, а позиция
 * считается от ВИЗУАЛЬНОГО якоря — иначе символ уползает из-под курсора. Повёрнутая
 * ФИГУРА ручек не получает: они тянут оси независимо, а геометрия локальная.
 */
