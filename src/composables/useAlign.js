...
/**
 * Выравнивание выделенных ячеек по общей рамке — БЕЗ наложений.
 *
 * Ячейки, пересекающиеся по перпендикулярной оси, образуют «полосу» и пакуются вплотную
 * вдоль оси выравнивания: сводятся до касания, но не налезают. Полоса из одной ячейки =
 * обычное выравнивание по краю или центру.
 *
 * Сдвиг идёт через `cell.translate(dx, dy, { uiNudge: true })` — тот же путь, что у
 * стрелок; в конце один bumpVersion и requestSnapshot, провода едут за портами. Края
 * остаются на сетке сами, у центров к сетке снапится старт полосы.
 */
