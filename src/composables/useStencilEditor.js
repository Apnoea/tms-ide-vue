/**
 * Модель редактора символов: черновик (мета, примитивы, порты, анимация состояния),
 * операции над ним и сборка артефактов. Только состояние и чистая логика — жесты и
 * DOM держит компонент StencilEditor.
 *
 * Синглтон: один черновик на стол и панель свойств (`createStencilEditor` — фабрика
 * для тестов). Вход — `reset()` для нового символа, `loadStencil()` для правки.
 */
...
/**
 * Шаг ГАБАРИТА символа при обрезке по контенту. Кратность 10 держит центр поворота
 * (w/2, h/2) на сетке: при ширине 15 центр 7.5 увёл бы порты на полуклетку.
 */
...
export function createStencilEditor() {
  ...
  // animationTemplate; включён — режим задаёт `stateMode`.
  const meta = reactive({
    id: '',
    label: '',
    category: '',
    ...
    stateColors: {},
    // Зоны диапазонов [{ min, max, color }] НЕЗАВИСИМЫ от состояний: символ разом
    // показывает положение по своему слоту и красится по числу другого тега. Тег зон
    // привязывают на холсте (слот `range`), здесь — только границы и цвета.
    ranges: [],
    // Символ программный (`locked` в определении): в редакторе доступны только зоны.
    locked: false,
    // Сколько имён портов выдано. Имя — ВЕЧНЫЙ ключ: за него держится провод и оно
    // уезжает в `data-tms-meta`, поэтому имена не переиспользуются.
    portSeq: 0,
  })
  const shapes = ref([])
  const ports = ref([])
  ...
  // Порты выделяются СВОИМ списком: у них нет ни свойств, ни порядка наложения, ни
  // копирования. Выделения взаимно исключающие — Del всегда однозначен.
  ...
  // состояния. В синглтоне: селектор рисует инспектор, фигуры фильтрует StencilEditor.
  ...
  // ввод, пипетка) — вызывающий на конце жеста.
  ...
  /**
   * Отличается ли черновик от ИСХОДНОГО состояния (снимок №0). Не `canUndo`: правка,
   * отменённая руками, оставляет шаги в истории при нулевой разнице.
   *
   * Сравнение сериализацией, как в `commit`. Разойтись строки могут лишь в сторону
   * «есть изменения» — безопасную: максимум лишний вопрос при закрытии.
   */
  ...
  /**
   * Сдвиг выделенных портов стрелками шагом сетки схемы: каждый проецируется на свою
   * ближайшую грань (`portOnEdge`), иначе общий сдвиг увёл бы часть внутрь тела.
   */
  ...
  /**
   * Сдвиг выделения на dx/dy (шаг задаёт вызывающий). Пачка едет ОДНИМ смещением:
   * поштучный снап развалил бы взаимное расположение фигур. У края шаг обрезается, а
   * не отменяется; история — один шаг на нажатие.
   */
  ...
  // Свойство есть не у каждого примитива (у линии нет заливки, у круга — скругления),
  // поэтому чтение и правка идут через ОДИН предикат применимости: контрол показан,
  // если свойство применимо хоть к одной выделенной фигуре.
  ...
  /**
   * Порядок наложения = порядок в массиве (он же порядок в экспортном SVG), поэтому
   * «выше/ниже» — перестановка списка общей с холстом `reorderIds`.
   *
  ...
  /**
   * Размер холста символа. Порты с грани переезжают на новую границу: координата вдоль
   * грани сохраняется, поперёк становится краем — иначе вывод остался бы внутри тела.
   * Размер снапится к BOX_GRID, тому же шагу, которым `cropToContent` обрезает габарит
   * на сохранении. Фигуры не трогаем — торчащий контент подсвечивает `contentOverflow`.
   *
   ...
   * что размер холста — верстак. Счёт общий с экспортом, цифра совпадает с файлом.
   */
   ...
   * объединение холста и будущего габарита.
   */
  ...
  /**
   * Порты с совпавшими координатами сводятся к одному (остаётся первый — у него
   * меньший номер в авто-именах): стопка выводов в одной точке выглядит как один порт,
   * а провод цепляется к невидимому соседу.
   *
  ...
  function loadStencil(def, { asCopy = false } = {}) {
    ...
    // Режим опознаётся по слоту-драйверу (`stateSlotOf` — не-`Text` и не `range`);
    // `states` — второй признак value-режима. Ключ слота сохраняется как есть:
    // переименование сломает привязку у расставленных экземпляров.
