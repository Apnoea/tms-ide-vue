...
/**
 * Рисование фигур-разметки на холсте. Жесты копируют редактор символов (там же
 * живёт эталон): rect/эллипс/линия тянутся drag'ом, ломаная набирается кликами и
 * замыкается кликом рядом со стартовой вершиной, подпись ставится одним кликом —
 * её габарит задаёт шрифт, а не рамка.
 *
 * Модель у фигур своя (`tms.Shape`, см. stencils/shapeElement), поэтому это отдельный
 * композабл, а не ветка в usePaletteDrag: там создаются ячейки-символы из реестра.
 *
 * Слушает pointerdown в capture-фазе: рисовать нужно и поверх существующих символов
 * (рамка вокруг группы — главный сценарий), а JointJS иначе начнёт drag ячейки.
 */

// Стиль новой фигуры: разметка контурная — заливки и скругления у неё нет
// (serializeShape подставит fill=none сам).
...
export function useCanvasDraw(paperContainer, { scheduleSnapshot }) {
  ...
  // Превью в координатах контейнера — рисуется overlay-SVG в CanvasPane.
