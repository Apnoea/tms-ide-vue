...
export function useProject({
   ...
   * IndexedDB → если в бандле есть символы, шлём их в dev-плагин
   * (он пишет в definitions/ → Vite авто-reload, на ребуте restoreProject поднимет
   * всё из IDB). Символов нет → применяем активную форму сразу. Предупреждаем о
   * символах, которых нет ни в базе, ни в бандле.
   */
  async function applyImportedBundle(data, graph, paper, projectName = null) {
    // Бандл-символы регистрируем в рантайме ДО парсинга — иначе parseSvgProject
    // выкинет их ячейки. Регистрируем не только новые (которых нет в реестре), но и
    // ИЗМЕНЁННЫЕ: если проект принёс другую версию существующего символа (правка
    ...
    // такой символ не попадёт ни в реестр, ни в оверрайды IDB, а его ячейки
    ...
    // Символы, на которые ссылаются формы (по meta SVG), но которых нет ни в базе,
    // ни в бандле — отрисовать их нечем, предупреждаем.
    // По ВСЕМ символам бандла, включая отклонённые: про них уже сказано отдельным
    ...
    // Грузит активную форму в граф. Символы (включая бандл-новые) уже в рантайм-
    ...
    // успех и НЕ пишем символы (их reload подхватил бы вместе с уже неполным проектом).
    if (!persisted) {
      applyActiveForm()
      notify.error(
    ...
    // Оверрайды символов проекта (новые + изменённые встроенные) → в IDB: они
