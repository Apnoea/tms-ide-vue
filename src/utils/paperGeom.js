...
/**
 * Проекция точки из model-координат в экранные (container-px) по текущему zoom/pan
 * paper'а — общая формула для overlay-композаблов.
 *
...
/**
 * Точка ячейки, ПОВЁРНУТАЯ вместе с ней: `angle` вращает outer-группу вокруг центра,
 * поэтому визуальное место локальной точки ≠ её модельные координаты. Обратный
 * поворот — тот же вызов с отрицательным углом.
 *
...
// (20×20) и без отступа перекрывала бы его и соседей.
const BTN_HALF = 16
const BTN_GAP = 24

/**
 * Позиции overlay-кнопок вокруг рамки выделения (css-`left`/`top` по центру кнопки).
 * Рамка задаётся в ПИКСЕЛЯХ, поэтому формула одна на холст и редактор символов.
 *
...
/**
 * Осевыровненный bbox ячейки С УЧЁТОМ поворота: `cell.getBBox()` отдаёт неповёрнутую
 * рамку (position+size), и у развёрнутого символа она врёт — отсюда щели при
 * выравнивании и смещённые overlay. Углы кратны 90°, поэтому результат округляем
