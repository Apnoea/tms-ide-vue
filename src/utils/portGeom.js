 ...
 *
 * Одна формула на трёх потребителей (отцепление конца провода, индекс портов при
 * импорте, врезка символа в линию) — разойдись копии, провода приклеятся не туда.
 * Принимает и модель JointJS, и её JSON: поля те же, читаются по-разному.
 */
...
/**
 * Точка одного порта. Порта с таким id нет → ЦЕНТР ячейки: так конец провода,
 * потерявший порт, остаётся на символе, а не улетает в (0,0). null — не ячейка.
 */
