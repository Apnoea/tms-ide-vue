...
/**
 * Версия набора символов: `1`, `1.2`, `1.2.3` — сравнивается покомпонентно числами.
 * Id набора своей маски не имеет: он ПРЕФИКС id его символов (`demo` → `demo_qw`),
 * поэтому проверяется тем же `STENCIL_ID_RE`.
 */
export const PRESET_VERSION_RE = /^\d+(\.\d+){0,2}$/

/**
 * Допустимый id формы. Тоже не косметика: id формы = имя ПАПКИ в архиве
 * (`forms/<id>/view.svg`), цель навигации (`tms.navigation` → view-id рантайма) и
 * ключ формы в проекте. Точки и слэши в нём дали бы путь наружу папки проекта при
 * распаковке архива на объекте, поэтому маска одна на все три роли: создание/
 * переименование формы, импорт чужого .zip и сборка исходящего архива.
 */
export const FORM_ID_RE = /^[A-Za-z0-9_-]+$/

/** Предел длины id формы: имя папки в архиве, ФС такое переживёт с запасом. */
export const FORM_ID_MAX = 64

/**
 * Имя формы из чужого архива → безопасный id. Недопустимые символы (пробелы, точки,
 * слэши) заменяем на `_`, а не отбрасываем форму: потерять схему хуже, чем
 * переименовать её.
 *
 * Имя, которое почти целиком из недопустимых символов (кириллица, `..`), чинить
 * бессмысленно — «Схема1» превратилась бы в «_____1», что не говорит автору ничего.
 * Такие отдаём пустой строкой: вызывающий даст осмысленное `form_N`.
 */
export function safeFormId(raw) {
  const s = String(raw ?? '')
  const kept = s.replace(/[^A-Za-z0-9_-]/g, '')
  if (!kept || kept.length * 2 < s.length) return ''
  return s
